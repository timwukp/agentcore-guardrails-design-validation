...
**Second editorial rule, added in this revision — the control's nature decides the verb.** Two tiers of claim appear in this document and they may not borrow each other's language. **Deterministic** controls — IAM deny, network egress deny, Cedar authorization, resource policies, SCPs — may be said to *prevent* something, scoped to a stated threat model, because they prevent the consequence whether or not an attack succeeds. **Probabilistic** controls — guardrail content filters, PII detection, prompt-attack detection — get *reduces* or *detects*, a measured efficacy figure, and a **residual-risk** statement; they never get *prevents*. This is not house style. OWASP LLM01:2025 states it is unclear whether fool-proof prompt-injection prevention exists and pivots to mitigating the *impact*; AWS's own guidance is titled "Detect prompt attacks" and uses only detect/filter/block language; and NIST AI RMF Playbook MANAGE 1.4 requires that residual risk be documented and disclosed. Where an earlier revision of this document used a prevention verb on a probabilistic control, the text has been corrected and the residual risk stated — §4.2 and §7.3 were the two sites.
...
- **Reduces** sensitive-data leakage through tool interactions — it does not prevent it, and this study did not measure the reduction. **Residual risk:** the `suppressOutput` effect was never observed suppressing anything here. F1-17, the case that probes it, is **INCONCLUSIVE**, and its instrument read the botocore service models for `bedrock`, `bedrock-runtime`, `bedrock-agentcore-control` and `bedrock-agentcore` locally in ten directions — *no operation was invoked*. So the effect's acceptance is an API-surface fact and its behaviour is untested. The PII detection this hop depends on is separately known to be incomplete: 9 of 31 documented entity types were refuted [corrected per F3-4, FALSE, 2026-08-10]. And no case in this study measured prompt-attack detection on tool **outputs** at all. Treat Hop #5 as defence in depth *behind* a deterministic control (Cedar `forbid`, or egress deny), never as the leakage boundary itself.
...
| **Tool I/O Guardrails** | Sensitive Info (PII on tool responses); Prompt Attack on external/untrusted tool responses | **Reduces** — does not prevent — data leakage and indirect prompt injection from tool outputs, and **the reduction is unmeasured in this study**: no case observed `suppressOutput` suppressing output (F1-17 INCONCLUSIVE, static API-surface read with no operation invoked) and no case measured prompt-attack detection on tool outputs. A configuration recommendation, not a verified control; see §4.2 for the residual risk |
...
**v1.4 — corrections driven by external standards and by the absence of evidence, not by any verdict:**

23. §4.2 BP#1 (Hop #5 bullet) and §7.3 (Tool I/O Guardrails row) — the verb **prevent** replaced by **reduce**, each site now carrying an explicit residual-risk statement. This is a framing correction, and it amends **no claim**: the two sentences asserted a security *outcome* this study never measured, and the correction does not substitute a measurement, it states the gap. Three facts fix the wording. First, the only case that goes anywhere near the mechanism is **F1-17, INCONCLUSIVE** — its sealed oracle reads "TRUE if a policy using `suppressOutput` is accepted and suppresses tool output; FALSE if the effect is rejected", and its instrument read the botocore **service models** for `bedrock`, `bedrock-runtime`, `bedrock-agentcore-control` and `bedrock-agentcore` locally in ten directions, so **no operation was invoked and nothing was ever observed suppressing anything**. Second, a grep over `lib/oracle.py` returns **zero** occurrences of `suppressOutput` and **zero** of `indirect`: no case in this study measures tool-output suppression, and no case measures prompt-attack detection on tool **outputs** at all — the closest measurement, F5-6, reports prompt-attack recall **0 [0, 0.031]** at n=120 on untagged `InvokeModel` **input**, which is a different channel and points the wrong way. Third, on the data-leakage half, F3-4 (FALSE, 2026-08-10) refuted **9 of 31** documented entity types, so even the deterministic-looking PII component does not cover what the bullet implies. The external warrant is that a prevention verb on a probabilistic filter is the defect OWASP LLM01:2025 names explicitly — it states it is unclear whether fool-proof prompt-injection prevention exists and pivots to mitigating **impact** — that AWS's own guidance page is titled "**Detect** prompt attacks" and uses only detect/filter/block language, and that NIST AI RMF Playbook MANAGE 1.4 requires residual risk to be documented and disclosed rather than implied away. Guidance consequence, stated at both sites: treat Hop #5 as **defence in depth behind** a deterministic control (a Cedar `forbid`, or an egress deny), never as the leakage boundary itself. Three other occurrences of the word in this document are correct and were deliberately left alone — §3.1's explicit detect-versus-prevent contrast ("a CloudTrail-based alarm *detects* the change rather than prevents it"), §3.2's random-`tagSuffix` tag-injection note, and §3.3's timeout-and-circuit-breaker item — each of which either names a deterministic control or uses the verb about something other than a filter's efficacy. The Traditional Chinese edition carries the identical change at the same two sites.
