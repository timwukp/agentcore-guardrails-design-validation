...
| **Field** | **Value** |
|:---|:---|
| **Version** | 1.3-DRAFT (empirically amended; validation incomplete) |
| **Date** | August 13, 2026 (v1.2: August 8, 2026) |
...
- **73** published verdicts: **43 TRUE** (claims empirically confirmed — annotated in place), **18 FALSE** (claims refuted — corrected in place), **11 INCONCLUSIVE** (instrument did not reach a usable state — claims left exactly as v1.2 wrote them), **1 RECORDED** (F5-4a, a descriptive failure-mode characterization used only to sharpen confirmed claims).
- **19** cases outstanding as of this draft: F1-6, F1-15, F1-19, F1-24, F1-25, F1-26, F1-27, F1-28, F5-3a, F5-3b, F5-4b, F5-5, F5-7b, F5-8, F5-9, F8-1, F9-3, F10-1, F10-3. Every claim these test is left exactly as v1.2 wrote it.

Editorial rule applied throughout: `[verified …]` marks a claim a published TRUE verdict confirms; `[corrected per …]` marks text rewritten because a published FALSE verdict refuted it; `(test pending …)` marks a claim whose case has no published verdict — the claim itself is untouched. An INCONCLUSIVE verdict is **not** evidence against a claim.

<!-- REVIEW: Verdicts for F1-6 (=TRUE), F8-1 (=FALSE), and F10-3 (=FALSE) were reported to the editor as having landed on 2026-08-13, but no results/phase1/F1-6.json, F8-1.json, or F10-3.json exists anywhere in the repository, and census.py, RECONNECT.md, and results/DEPENDENCY-AUDIT-2026-08-13.md all still list all three as outstanding. Per the rule that an amendment a reader cannot trace to a result file is worse than no amendment, NO change was made on the basis of those three reported verdicts. If the result files land, the affected sites are: §1 Regional-availability callout + §8 Region check (F8-1); §3.4 tier table crossRegionConfig row + §1 language callout (F1-6); §3.3 BP#3 + Appendix B "Selective application" row (F10-3). -->
...
- Fail-secure: Policy evaluation timeouts result in an automatic DENY decision (AgentCore Service Approval Accelerator v2.9, Policy section). *(Validation: untestable from outside AWS — no fault-injection surface exists for service-side evaluation timeouts; case F9-1 is excluded by its own sealed oracle. Claim left as written.)*
- Denied requests receive a message identifying the denying policy ID — **but not HTTP 403 on MCP targets** [corrected per F4-6, FALSE, n=120, us-east-1, 2026-08-10]: all 120 measured policy denials on an MCP gateway target returned **HTTP 200** with a JSON-RPC error (code -32002, "Tool Execution Denied: Tool call not allowed due to policy enforcement […]") naming the denying policy ID in the message. Do not key alerting or client logic on a 403 status for MCP traffic; parse the JSON-RPC error instead.
...
3. Apply selective evaluation — not all inputs require full guardrail assessment (input tagging also enables evaluating only the user-supplied portion of a RAG prompt, reducing text units billed) *(test pending: the text-unit-reduction claim is case F10-3, which has no published verdict as of 2026-08-13 — claim left as written)*
...
| Cross-Region inference | Not used | **Required** (`crossRegionConfig` / guardrail profile) *(test pending: F1-6 has no published verdict as of 2026-08-13 — row left as written)* |
...
- Word filters support English, French, and Spanish only, on either tier. *(Validation: case F8-7 returned INCONCLUSIVE and F1-26 is outstanding — claim left as written.)*
...
- Policy/guardrails apply to three gateway target types — MCP targets (`POST /mcp`, JSON-RPC `tools/call`), HTTP runtime targets (`POST /<target>/invocations`), and HTTP inference targets (`POST /inference`) — not MCP tools only *(test pending: case F1-15 has no published verdict as of 2026-08-13 — claim left as written)*
- Fail-secure: evaluation timeouts result in automatic DENY *(Validation: service-side timeouts are untestable from outside AWS — F9-1 excluded by its sealed oracle; claim left as written)*
...
**Limitations of guardrails in policy:** (1) no regex or pattern matching — guardrails use ML scoring, not regular expressions; (2) you cannot mix standard Cedar `when {…}` with `when guardrails {…}` — the guardrails block replaces the standard condition; (3) a `when guardrails {…}` block must contain at least one guardrail definition.
...
4. PII caveat: the Bedrock Guardrails sensitive-information filter does not detect PII inside `tool_use` output parameters — do not treat Hop #5 as a complete PII barrier for structured tool arguments; validate structured parameters with Cedar or interceptors instead
...
- Note: reasoning/chain-of-thought content blocks are excluded from guardrail evaluation *(test pending: case F1-27 has no published verdict as of 2026-08-13 — claim left as written)*
...
| **Ignoring guardrail throttling** | Silent failures under load | Monitor InvocationThrottles and implement retry/backoff |
...
- [ ] **Verify tier vs. traffic language** (Section 3.4): non-EN/FR/ES traffic REQUIRES Standard tier — Classic is silently ineffective [verified F8-2, TRUE, n=240, 2026-08-10]; confirm data-residency allows Standard's mandatory cross-Region inference *(the `crossRegionConfig` requirement is test pending — F1-6 unpublished as of 2026-08-13)*. **Also test your own denied-topic definition lengths:** the documented Standard-tier 1,000-char limit was rejected with `ValidationException` [F8-5, FALSE, 2026-08-10]
- [ ] Set up AgentCore Gateway with Policy Engine — confirm Guardrails-in-policy is available in your Region *(test pending: the Regional-availability probe F8-1 has no published verdict as of 2026-08-13 — item left as written)*
...
flowchart TD
    ...
    subgraph DURING["DURING — Execution Control"]
        ...
        H4["Hop #4: Cedar Tool Auth<br/>(AgentCore Policy)<br/>measured p50 55ms / p90 70ms<br/>default-deny · fail-secure (timeout case untested)"]
...
| **PII/Sensitive Info** | ❌ | ✅ (detect — **but 9 of 31 documented entity types were measured undetected; see §3.2** [F3-4, FALSE, n=341, 2026-08-10]) | ✅ (redact; note tool_use parameters are not scanned *(test pending: F1-28 unpublished as of 2026-08-13)*) | ✅ (redact) |
| **Denied Topics** | ❌ | ✅ | ❌ | ✅ |
| **Word Blocklist** | ❌ | ✅ | ❌ | ✅ |
...
| **Selective application** | Only apply guardrails to content that needs evaluation; input tagging scopes evaluation to user content | Proportional to skip rate *(test pending: the text-unit/billing effect of tagging is case F10-3, which has no published verdict as of 2026-08-13 — row left as written)* |
...
## Appendix D: Change Log v1.2 → v1.3-DRAFT (empirical amendments)

Every change in v1.3 is traceable to a published verdict file under `results/phase1/` in the `grx-validation` repository. Claims whose cases are outstanding, INCONCLUSIVE, or untestable are **unchanged** — only markers were added. Counts re-derived via `census.py` on 2026-08-13.
...
13. §3.1 — **new SDK prerequisite:** `botocore`/`boto3` ≥ 1.43.32, with the 1.43.30–.31 trap window named; also added to the §8 checklist [F1-1, F1-2, TRUE, 2026-08-09].
14. §3.1 default-deny callout, §7.1 step 4, §7.2, §8 — **new `validationMode` requirement:** the recommended baseline permit statement settles `CREATE_FAILED` under the default `FAIL_ON_ANY_FINDINGS`, asynchronously, after a 202 [F1-3, TRUE, replicated 2026-08-10/11].
15. §3.1 BP#5 — **new measured interval:** an accepted mode flip took 602.8/931.7 ms and a previously-blocked request was served 13.2–14.2 s later; `iam:PassRole` named beside `UpdateGateway` [F5-2, TRUE, replicated 2026-08-12/13].
16. §4.4 route #3 — **least privilege rescoped to the steady state:** revocation is eventually consistent in both directions (32 of 80 invocations executed after an observed denial; control-plane authorization persisted 325.0 s / 305.8 s after grant deletion), so runbooks of the form "revoke, confirm, proceed" are prohibited and no wait-N-seconds number is published [F5-1 and F5-2, both TRUE, replicated across four UTC days].
17. §6.4 — **new publish-lag floor:** measured p90 lag 11.5 s; only 1 of 7 alarms states a period, and the other 6 are unimplementable as written [F7-6, TRUE, n=30, 2026-08-10].
18. §6.3 — actual span names recorded (`AgentCore.Policy.AuthorizeAction`, `AgentCore.Gateway.InvokeTool[.<tool>]`), ≈50 s span lag, and the measured absence of a score attribute on spans [F7-4, TRUE, n=20, 2026-08-10].
19. In-place confirmations added (prose unchanged, citation appended): §1 and §3.4 language/tier claims [F8-2, F8-3, F8-6, F8-8]; §3.1 score lattice [F1-18], content-filter categories [F1-7]; §3.2 detection efficacy [F3-1, F3-2, F3-3, F3-5, F3-6, F3-7] and input tagging [F5-6]; §4.1 determinism, default-deny, forbid-override and the two-level LOG_ONLY precedence [F2-1, F4-2, F4-3, F4-4, F4-5, F1-5]; §4.4 bypass route #1 [F5-1]; §5.2/§5.3 capability inventories [F1-22, F1-23]; §6.1 additivity [F6-7] and end-to-end total [F6-6]; §6.2 metric namespaces and batching [F7-2, F7-3, F7-7] and per-text-unit billing [F10-2]; §7.1 principle #2 early-block saving [F6-9]; §10 references [F0-1].

**Deliberately left unchanged (evidence inconclusive, absent, or untestable):** §3.2 BP#1 per-direction independence (F1-11 INCONCLUSIVE); §5.1 BP#5 Contextual Grounding character limits (F1-13 INCONCLUSIVE); §3.4 word-filter language claim (F8-7 INCONCLUSIVE, F1-26 outstanding); §3.1 prompt-attack subtype enumeration (F1-8 INCONCLUSIVE); §5.1 BP#1 streaming modes (F1-12 INCONCLUSIVE); §4.1 interceptors (F1-16 INCONCLUSIVE) and `suppressOutput` effect (F1-17 INCONCLUSIVE); §3.3/Appendix B 10-content-block cap (F1-20 INCONCLUSIVE); §3.2 auto-update/drift guidance (F3-11 INCONCLUSIVE); §1 and §8 Regional availability (F8-1 outstanding); §3.4 `crossRegionConfig` requirement (F1-6 outstanding); §3.2 and §7.1 billing asymmetry (F10-1 outstanding); §3.3 BP#3 and Appendix B tagging/text-unit saving (F10-3 outstanding); §4.1 three target types (F1-15 outstanding); §5.1 reasoning-block exclusion (F1-27 outstanding); §4.2 BP#4 and Appendix A `tool_use` PII caveat (F1-28 outstanding); §3.1/§4.1/§9 fail-secure timeout → DENY (F9-1 untestable by sealed oracle; F5-4b outstanding); §4.5 network containment and §4.4 routes #2 and #4 (F5-3a, F5-3b, F5-7b, F5-8 outstanding); §4.2 BP#1 indirect prompt injection (F5-5 outstanding); §7.2 throttling anti-pattern (F9-3 outstanding); §3.1 threshold defaults (F1-19 outstanding); §4.1 guardrail-in-policy limitations (F1-24, F1-25 outstanding).
