// One renderer for every diagram the payload carries, whichever page draws it.
//
// WHY THIS IS A COMPONENT AND NOT A SECOND COPY
//
// Two pages draw diagrams out of `architecture.json`: `/architecture` draws what the study measured,
// `/design` draws the recommended closed loop. Both need the same geometry layer, the same five status
// colours, the same arrowheads and the same panel. A second renderer beside this one would be two
// readers of one format, and the way that fails is not that one breaks — it is that one keeps working
// while the other drifts, so the picture a reader screenshots is drawn by whichever file was edited
// last (`feedback_two_readers_one_format`). The page decides WHICH diagrams to draw, by filtering on
// the payload's own `view` key; it does not decide how one looks.
//
// WHY THIS IS RENDERED FROM COORDINATES IT DOES NOT COMPUTE
//
...
import { A, T, useAuthored, useT } from "../lib/i18n";
import type { ArchBox, ArchDiagram as ArchDiagramData, ArchEdge, Architecture } from "../lib/types";
...
function Panel({ box, arch }: { box: ArchBox; arch: Architecture }) {
  const t = useT();
  // Resolved here rather than rendered through `<A>` because each of these needs its `lang` on an
  // element `<A>` does not own — a heading, and several paragraphs one of which keeps its newlines. The
  ...
  const whyStatus = authored(box.why_this_status);
  ...
  return (
    <div className={`archpanel ${statusClass(box.status)}`}>
      ...
      {/* `detail` and `why_these_cases` are the authored topology file's own sentences, quoted rather
          than paraphrased for the same reason `oracle_text` is — a second wording of a claim about a
          component is a claim whose only provenance is the wording.

          `status_label` and `why_this_status` are NOT among them, and this comment named the first as
          one until 2026-08-22 and rendered the second in a hardcoded `lang="en"` until 2026-08-25. Both
          are this platform's own prose: `ARCH_STATUS_LABEL[box.status]` says what a colour means, and
          `box_status()` composes the sentence that derives it from the case ids. The second is the most
          rendered sentence the build writes — 50 boxes, 9 checkpoints and 45 practices resolve their
          colour through it — and it is exactly the paragraph a reader needs in order to DISTRUST the
          colour, so leaving it English gave the reader who needed it least the only copy of it. */}
      <p className="what" lang={detail.lang}>
        {detail.text.trim()}
      </p>
      <p className="why">
        <strong>{t("arc.whyColour")}</strong> <span lang={whyStatus.lang}>{whyStatus.text}</span>
      </p>
      <p className="why">
      ...
      <table className="grid archkv">
        <tbody>
          ...
          {/* Only the closed-loop diagram sets this: the checkpoint section of the design document a
              box takes its cases from. It is a link because the practices that cite those cases are on
              another page, and a reader looking at a contested hop asks what was recommended there. */}
          {box.from_section ? (
            <tr>
              <th>{t("arc.kv.section")}</th>
              <td>
                <Link to={`/design#s-${box.from_section}`} className="mono">
                  §{box.from_section}
                </Link>
              </td>
            </tr>
...
export default function ArchDiagram({ d, arch }: { d: ArchDiagramData; arch: Architecture }) {
